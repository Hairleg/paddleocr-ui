...
def process_pdf(
    ...
    **kwargs,
    ...
    for page_num in range(total_pages):
        ...
        stamp_elements = []
        try:
            from app.pipeline.core.layout.stamp_detector import detect_all_stamps
            stamps = detect_all_stamps(img)
            if stamps:
                stamp_dir = os.path.join(output_dir, "images", "stamps")
                os.makedirs(stamp_dir, exist_ok=True)
                for stamp in stamps[:5]:  # Max 5 stamps per page
                    stamp_file = os.path.join(stamp_dir, f"page{page_num+1:04d}_stamp{len(stamp_elements)+1:02d}.png")
                    sx, sy, sw, sh = stamp.bbox
                    cropped = img[max(0,sy-8):min(img.shape[0],sy+sh+8), max(0,sx-8):min(img.shape[1],sx+sw+8)]
                    cv2.imwrite(stamp_file, cropped)
                    scale = 72.0 / dpi
                    stamp_elements.append(PageElement(
                        type=ElementType.STAMP,
                        bbox=(int(sx*scale), int(sy*scale), int(sw*scale), int(sh*scale)),
                        image_path=stamp_file,
                    ))
        except Exception as exc:
            logger.debug("Stamp detection skipped on page %d: %s", page_num + 1, exc)

        # ── Image region detection (controlled by image_level) ──
        image_elements = []
        if image_level >= 1:
            try:
                from app.pipeline.core.layout.image_detector import detect_image_regions, crop_and_save_images
                img_regions = detect_image_regions(img, image_level)
                if img_regions:
                    saved_paths = crop_and_save_images(img, img_regions, output_dir, page_num + 1, dpi)
                    for r_idx, (region, path) in enumerate(zip(img_regions, saved_paths)):
                        x, y, w, h = region["bbox"]
                        scale = 72.0 / dpi
                        image_elements.append(PageElement(
                            type=ElementType.IMAGE,
                            bbox=(int(x*scale), int(y*scale), int(w*scale), int(h*scale)),
                            image_path=path,
                        ))
                    logger.info("Page %d: extracted %d image regions", page_num + 1, len(image_elements))
            except Exception as exc:
                logger.debug("Image detection skipped on page %d: %s", page_num + 1, exc)

        # ── Build page layout with text, tables, and stamps ──
        ...
        # OCR-based text elements (filter out text inside table regions)
        if page_ocr is not None:
            n_texts = len(page_ocr.get("rec_texts", [])) if hasattr(page_ocr, 'get') else 0
            text_elements = _build_text_elements_from_ocr(page_ocr, width_pt, height_pt, dpi, table_regions)
            logger.info("Page %d: OCR texts=%d elements=%d tables=%d",
                       page_num + 1, n_texts, len(text_elements), len(table_regions))
...
def _build_text_elements_from_ocr(
    ...
    for i, text in enumerate(rec_texts):
        ...
        # No filtering — keep all text. Table cells are handled separately.
        confidence = rec_scores[i] if i < len(rec_scores) else 0.0
        font_size_pt = max(8, min(72, round(ph * 72.0 / dpi / 1.2)))
...
def _extract_electronic_page_with_tables(
    ...
    # ── Step 1b: PyMuPDF find_tables supplement (text-precise cell content)
    if enable_table:
        try:
            found_tables = page.find_tables(strategy=strategy)
            if found_tables:
                for t_idx, ft in enumerate(found_tables):
                    tb = ft.bbox
                    # Skip if overlaps existing YOLO table
                    overlaps = False
                    for ex, ey, ex2, ey2 in table_bboxes:
                        ox = max(0, min(tb[2], ex2) - max(tb[0], ex))
                        oy = max(0, min(tb[3], ey2) - max(tb[1], ey))
                        if ox > 0 and oy > 0: overlaps = True; break
                    if overlaps: continue

                    # PyMuPDF text extraction (preferred for electronic PDFs —
                    # renders are clean vector text, but OCR loses quality vs
                    # native text layer extraction. RapidTable is only used
                    # for scanned pages where we have no text layer.)
                    tdata = None
                    rows = ft.extract()
                    if rows and len(rows) >= 1:
                        tdata_rows = []
                        for r_idx, row_texts in enumerate(rows):
                            cells = [TableCell(row=r_idx, col=c_idx,
                                    text=(text or "").strip())
                                    for c_idx, text in enumerate(row_texts)]
                            tdata_rows.append(cells)
                        sheet_name = f"第{page_num}页_表格{t_idx+1}"
                        tdata = TableData(rows=tdata_rows, title="", sheet_name=sheet_name)

                    if tdata:
                        tables.append(tdata)
                        elem = PageElement(
                            type=ElementType.TABLE,
                            bbox=(int(tb[0]), int(tb[1]), int(tb[2]-tb[0]), int(tb[3]-tb[1])))
                        elem._table_data = tdata
                        elements.append(elem)
                        table_bboxes.append((tb[0], tb[1], tb[2], tb[3]))
        except Exception as exc:
            logger.debug("find_tables failed on page %d: %s", page_num, exc)
