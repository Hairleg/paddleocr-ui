...
- [x] **T-007** 前端超时上限 120→180 分钟
- [x] **T-008** 预检时估算耗时提示

### 性能
<!-- [x] **T-010** PDX 磁盘缓存调研 → 性能优化，非必须 -->
<!-- [x] **T-011** 含表崩溃 → PIR=Disabled 已解决，不复现 -->
...
### 格式优化
- [ ] **T-030** 段落合并过度（word_writer）:
  - 根因: `_merge_adjacent_paragraphs` 字号容差 4pt 过大，无位置检查
  - 修复:
    1. 字号容差 4pt → 1.5pt（保留正文/标题边界）
    2. 增加垂直间距检查: 间距 > 2×line_height → 不合并
    3. docx 保底: 即使字号相近，不同 `reading_order` 行不合并
  - txt: 继续当前激进合并，不调整
  - 文件: `app/pipeline/export/word_writer.py` L54-88

### 文档
- [x] **T-022** README 性能数据最终更新
- [x] **T-023** DEV_PITFALLS 补充 PIR 解决方案 + writer 大文档坑
